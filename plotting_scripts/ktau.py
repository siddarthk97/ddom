...
d = {'$\tau = 0.9$': np.asarray([87.310, 95.376,101.381]), '$\tau = 0.75$': np.asarray([87.349, 110.993,87.330]), '$\tau = 0.50$': np.asarray([87.396,94.153,85.397])}
...
sort_means_k = [103.600, 95.660, 88.667]
...
plt.xlabel("Supercon.", fontsize=FS)
plt.ylim([0, 120])
...
ax.legend((x[0] for x in rects), (r"$\tau =R_{90}$", r"$\tau =R_{75}$", r"$\tau =R_{50}$"), fontsize=FS-5, loc="best")# , ), fontsize=FS-5, loc="best")
