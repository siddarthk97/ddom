...
class DBTensorDatasetDataModule(AbstractDataModule):
    """Abstract class for Design-Bench datasets that can be stored as a TensorDataset."""
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        # action_tensor = torch.tensor(dataset["actions"])
        # dones = np.logical_or(dataset["terminals"], dataset["timeouts"])
        observation_tensor = torch.tensor(task.y)
        action_tensor = torch.tensor(task.x)
...
class DBDataModule(DBTensorDatasetDataModule):
    """Data module for RvS-R (reward-conditioned) learning in D4RL."""
